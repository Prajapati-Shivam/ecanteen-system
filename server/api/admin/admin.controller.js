...
const fetchItems = async (req, res) => {
    ...
    const { filter_name, filter_category, filter_type, filter_price, filter_availability, filter_sort } = req.query;
    ...
    try {
        ...
        const items = await Item.find(filter, { college_id: 0, __v: 0 }).sort(sort).skip(skip).limit(limit);
...
const updateItem = async (req, res) => {
    const { item_id, name, price, category, image, type, availability } = req.body;
